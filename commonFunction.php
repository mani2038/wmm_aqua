...
while($res = mysqli_fetch_array($res2))
{
	$companyname1['0'] = $res['companyname'];
	$companyname1['1'] = $res['ad'];	
}
...
//To calculate total Actions for the Lead number
function getCountForLead_retail($leadNumber,$dept) {
global $con;
$sql = "SELECT count(*) FROM oxy2_action_retail WHERE department = '$dept' AND leadNum = '$leadNumber'";
//echo "$sql";
$res2 = mysqli_query($con, $sql);
if (!$res2) {
   return mysqli_error();
...
//----------------------**Checking for PNL Download**-------------------------------------
function DownaloadPNL($leadno) {
    global $con;
$sql = "SELECT * FROM oxy_pnl WHERE leadId = '$leadno' ORDER BY id DESC";
...
$rowData = mysqli_fetch_array($res2);
return $rowData;
}
//--------------------**Table Details**-----------------------------------
...
$num_rows = mysqli_num_fields($res2);
$res = mysqli_fetch_array($res2);
